...
- `cmd_edit_lengths.go` — `edit-lengths`: edits lengths via `--set-idle`/`--set-stroke`/`--set-stroke-count`, `--merge`/`--merge-force`, or `--split` (mutually exclusive groups); recomputes affected laps and session. `--set-stroke-count` sets a length's `total_strokes` (active lengths only) and recomputes its `avg_swimming_cadence`. `--merge-force` behaves like `--merge` but waives the policy checks (same lap / same type / same stroke), keeping only the structural ones — used to merge orphaned idle lengths.
...
- A length merge relocates time, it never deletes it, so a lap that loses a length keeps its `TotalElapsedTime`/`TotalTimerTime` (the removed length's time becomes dead time). Without this, `--merge-force` moving a *counted* length out of its lap (e.g. consolidating the idle lengths of a rest whose lap counted one of them) would silently shrink the lap and drop activity time — and `verify` wouldn't catch it, since the result is still internally consistent.
- `--merge-force` only waives the policy checks (same lap / same type / same stroke). The merged length keeps the lower (kept) length's lap membership; the upper length's lap, if any, loses it. Merging two idle lengths where the kept one is orphaned therefore yields an orphaned idle length. Forcing a merge of two *active* lengths across a lap boundary is the caller's responsibility — well-defined but rarely physically meaningful.
